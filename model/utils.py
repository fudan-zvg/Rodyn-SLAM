...
def get_masks_new(z_vals, target_d, truncation):
    '''
    Params:
        z_vals: torch.Tensor, (Bs, N_samples)
        target_d: torch.Tensor, (Bs,)
    ...
    # after truncation
    back_mask = torch.where(z_vals > (target_d + truncation), torch.ones_like(z_vals), torch.zeros_like(z_vals))
    center_mask = torch.where((z_vals > (target_d - 0.4 * truncation)) *
                              (z_vals < (target_d + 0.4 * truncation)),
                              torch.ones_like(z_vals), torch.zeros_like(z_vals))

    # valid depth mask
    depth_mask = torch.where(target_d > 0.0, torch.ones_like(target_d), torch.zeros_like(target_d))

    tail_mask = (1.0 - front_mask) * (1.0 - back_mask) * (1.0 - center_mask)
    # Valid center and tail region
    center_mask = center_mask * depth_mask
    tail_mask = tail_mask * depth_mask

    num_fs_samples = torch.count_nonzero(front_mask)
    num_sdf_center_samples = torch.count_nonzero(center_mask)
    num_sdf_tail_samples = torch.count_nonzero(tail_mask)
    num_samples = num_sdf_center_samples + num_sdf_tail_samples + num_fs_samples

    fs_weight = 1.0 - num_fs_samples / num_samples
    sdf_center_weight = 1.0 - num_sdf_center_samples / num_samples
    sdf_tail_weight = 1.0 - num_sdf_tail_samples / num_samples

    return front_mask, center_mask, tail_mask, fs_weight, sdf_center_weight, sdf_tail_weight
...
def get_sdf_loss_new(z_vals, target_d, predicted_sdf, truncation, loss_type=None, grad=None):
    '''
    Params:
        z_vals: torch.Tensor, (Bs, N_samples)
        target_d: torch.Tensor, (Bs,)
    ...
    Return:
        ...
        eikonal_loss: torch.Tensor, (1,)
    '''
    front_mask, center_mask, tail_mask, fs_weight, sdf_center_weight, sdf_tail_weight = get_masks_new(z_vals, target_d, truncation)
    fs_loss = compute_loss(predicted_sdf * front_mask, torch.ones_like(predicted_sdf) * front_mask, loss_type) * fs_weight
    center_loss = compute_loss((z_vals + predicted_sdf * truncation) * center_mask, target_d * center_mask, loss_type) * sdf_center_weight * 20.0
    tail_loss = compute_loss((z_vals + predicted_sdf * truncation) * tail_mask, target_d * tail_mask, loss_type) * sdf_tail_weight * 5.0

    sdf_loss = center_loss + tail_loss
