...
class KeyFrameDatabase(object):
    def __init__(self, config, H, W, num_kf, num_rays_to_save, device) -> None:
        self.config = config
        self.keyframes = collections.OrderedDict()
        self.device = device
    ...
    def sample_single_keyframe_rays(self, rays, seg_mask, option='random'):
        '''
        ...
        if option == 'random':
            ...
            seg_mask = seg_mask[..., None].reshape([1, -1])
            valid_depth_mask = valid_depth_mask & seg_mask
            rays_valid = rays[valid_depth_mask, :]  # [n_valid, 7]
            num_valid = len(rays_valid)
            if num_valid < self.num_rays_to_save:
                idxs = random.sample(range(0, num_valid), num_valid)
                idxs_add = random.sample(range(0, num_valid), self.num_rays_to_save - num_valid)
                idxs = idxs + idxs_add
            else:
                idxs = random.sample(range(0, num_valid), self.num_rays_to_save)
            rays_valid = rays_valid[None, ...]
        elif option == 'filter_mask':
            seg_mask = seg_mask[..., None].reshape([1, -1])
            rays_valid = rays[seg_mask, :]  # [n_valid, 7]
            num_valid = len(rays_valid)
            if num_valid <= self.num_rays_to_save:
                idxs = random.sample(range(0, num_valid), num_valid)
                idxs_add = random.sample(range(0, num_valid), self.num_rays_to_save - num_valid)
    ...
    def add_keyframe(self, batch, filter_depth=False):
        ...
        seg_mask = 1-seg_mask
        if filter_depth:
            rays = self.sample_single_keyframe_rays(rays, seg_mask, 'filter_depth')
        else:
            rays = self.sample_single_keyframe_rays(rays, seg_mask, 'filter_mask')
        ...
        # Save keyframe
        self.keyframes[batch['frame_id']] = batch
    ...
    def sample_local_rays(self, bs, local_kf_num):
        '''
        Sample rays from self.rays as well as frame_ids
        '''
        num_kf = self.__len__()
        all_num_kf = self.__len__()
        start_kf = 0
        if num_kf > local_kf_num:
            num_kf = local_kf_num
            start_kf = all_num_kf - local_kf_num

        idxs = torch.tensor(random.sample(range(num_kf * self.num_rays_to_save), bs))
        sample_rays = self.rays[start_kf:all_num_kf].reshape(-1, 7)[idxs]
